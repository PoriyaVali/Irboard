...
class ThemeController extends Controller
{
    ...
    public function saveThemeConfig(Request $request)
    {
        ...
        if (!$payload['config'] || !is_array($payload['config'])) abort(500, 'پارامتر نامعتبر است');
        $themeConfigFile = public_path("theme/{$payload['name']}/config.json");
        if (!File::exists($themeConfigFile)) abort(500, 'قالب یافت نشد');
        $themeConfig = json_decode(File::get($themeConfigFile), true);
        if (!isset($themeConfig['configs']) || !is_array($themeConfig)) abort(500, 'فایل پیکربندی قالب نامعتبر است');
