...
class UserController extends Controller
{
    ...
    private function filter(Request $request, $builder)
    {
        ...
        if ($filters) {
            foreach ($filters as $k => $filter) {
                if ($filter['condition'] === '模糊' || $filter['condition'] === 'تقریبی') {
    ...
    public function getUserInfoById(Request $request)
    {
        ...
        $user['reserved_plans'] = \App\Models\ReservedPlan::where('user_id', $user->id)
            ->where('status', 0)
            ->with('plan:id,name')
    ...
    public function deleteReservedPlan(Request $request)
    {
        $id = $request->input('id');
        if (empty($id)) {
            abort(500, 'پارامتر نامعتبر');
        }
        $reserved = \App\Models\ReservedPlan::find($id);
        if (!$reserved) {
