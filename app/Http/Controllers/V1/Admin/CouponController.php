...
class CouponController extends Controller
{
    ...
    private function multiGenerate(CouponGenerate $request)
    {
        ...
        DB::commit();
        $data = "نام,نوع,مبلغ یا درصد,زمان شروع,زمان پایان,تعداد مجاز,قابل‌استفاده برای اشتراک,کد کوپن,زمان تولید\r\n";
        foreach($coupons as $coupon) {
            $type = ['', '金额', '比例'][$coupon['type']];
            $value = ['', ($coupon['value'] / 100),$coupon['value']][$coupon['type']];
            $startTime = date('Y-m-d H:i:s', $coupon['started_at']);
            $endTime = date('Y-m-d H:i:s', $coupon['ended_at']);
            $limitUse = $coupon['limit_use'] ?? 'نامحدود';
            $createTime = date('Y-m-d H:i:s', $coupon['created_at']);
            $limitPlanIds = isset($coupon['limit_plan_ids']) ? implode("/", $coupon['limit_plan_ids']) : 'نامحدود';
